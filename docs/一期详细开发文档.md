...
**需求来源：** 2026-03-27 产品评审
**问题描述：** 当前 DiffPanel 以"段落块"为粒度上色（整行背景变红/绿），无法直观看出段落内部哪个词语发生了变化。
**目标：** 在双栏文档内容中，对具体变更的字符/词语进行内联颜色标记（删除→红色删除线，新增→绿色高亮）。

### 24.1 方案一：前端字符级 Diff 高亮（已采纳，优先执行）

**技术选型：** `diff-match-patch`（Google 出品，支持 Unicode / 中文，MIT 协议）

**原理：**
```
doc_a_text ──┐
              ├── diff-match-patch ──→ [(-1,"10%"), (0,"作为"), (1,"20%")]
doc_b_text ──┘                                ↓ 渲染为 HTML
                              左栏: "~~10%~~作为违约金"  （红色删除线）
                              右栏: "**20%**作为违约金"  （绿色高亮）
```

**改动范围：** 仅 `frontend/src/components/compare/DiffPanel.vue`，新增 `diff-match-patch` npm 依赖。

**各 diff_type 渲染规则：**

| diff_type | 左栏（doc_a） | 右栏（doc_b） |
|-----------|-------------|-------------|
| `modify`  | 字符级红色删除线标记 | 字符级绿色高亮标记 |
| `delete`  | 整段红色背景 + 删除线 | 灰色"（无内容）" |
| `insert`  | 灰色"（无内容）" | 整段绿色高亮 |
| `move`    | 整段蓝色标记 | 整段蓝色标记 |
...
2. 在 `DiffPanel.vue` 中引入并替换 `renderText()` 逻辑：
   - `modify` 类型：调用 `dmp.diff_main(textA, textB)` → `dmp.diff_cleanupSemantic()` → 渲染为带颜色 span 的 HTML
   - 其他类型：沿用整块高亮逻辑（已有 `.diff-delete-mark` / `.diff-insert-mark` 样式）

3. 新增两个渲染函数：
   - `renderModifyA(textA, textB)` → 提取 DELETE 操作段，渲染左栏
   - `renderModifyB(textA, textB)` → 提取 INSERT 操作段，渲染右栏

**优点：** 不改后端 API、响应快（纯本地计算）、中文支持完善
**缺点：** 字符级比对为纯文本层面，与后端语义级比对结果可能有轻微出入
**完成状态：** ✅ 2026-03-27 已实现并提交

---

### 24.2 方案二：后端字符级差异数据（备选，精准度更高）

**适用场景：** 方案一精准度不满足需求时，升级为后端统一计算。

**技术选型：** Python `diff-match-patch` 库，在后端比对流水线中预计算字符级差异。

**后端改动：**

1. `DiffItem` ORM 模型新增字段：
   ```python
   inline_diff_a: Mapped[Optional[str]] = mapped_column(LONGTEXT, nullable=True)
   # 存储 JSON 字符串，格式：[[-1,"10%"],[0,"作为违约金"]]
   inline_diff_b: Mapped[Optional[str]] = mapped_column(LONGTEXT, nullable=True)
   # 格式：[[0,"作为违约金"],[1,"20%"]]
   ```

2. Pydantic Schema 同步新增 `inline_diff_a / inline_diff_b: list | None`

3. 在 `char_diff.py` 的 `modify` 类型处理逻辑中，调用：
   ```python
   import diff_match_patch
   dmp = diff_match_patch.diff_match_patch()
   diffs = dmp.diff_main(text_a, text_b)
   dmp.diff_cleanupSemantic(diffs)
   item.inline_diff_a = json.dumps([[op, text] for op, text in diffs if op != 1])
   item.inline_diff_b = json.dumps([[op, text] for op, text in diffs if op != -1])
   ```

4. 需要 Alembic 迁移脚本：`ALTER TABLE diff_items ADD COLUMN inline_diff_a LONGTEXT`

**前端改动：**

`DiffPanel.vue` 的 `renderText()` 优先使用 `item.inline_diff_a / inline_diff_b` 字段，字段为空时降级为方案一的前端计算。

**前置条件：** 需要 Alembic 迁移 + 后端接口版本升级，工作量约 1-2 天。
**完成状态：** 📋 待排期（一期可选，二期优先落地）
